...
public class PatternMatcherUtils {
	...
	public static boolean testCompatibilityGlobPattern(String pattern, String test) {
		...
		if (!StrUtil.startWith(pattern, SharingConstant.PATH_SEPARATOR)) {
			pattern = SharingConstant.PATH_SEPARATOR + pattern;
		}
		
		// 兼容性处理.
		test = StringUtils.concat(test, SharingConstant.PATH_SEPARATOR);
