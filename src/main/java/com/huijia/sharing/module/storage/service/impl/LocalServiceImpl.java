...
import com.huijia.sharing.module.storage.model.param.LocalParam;
...
public class LocalServiceImpl extends AbstractProxyTransferService<LocalParam> {
    ...
    public void uploadFile(String pathAndName, InputStream inputStream) {
        ...
        String uploadPath = StringUtils.removeDuplicateSlashes(baseFilePath + SharingConstant.PATH_SEPARATOR + pathAndName);
        // 如果目录不存在则创建
    ...
    public ResponseEntity<Resource> downloadToStream(String pathAndName) {
        ...
        File file = new File(StringUtils.removeDuplicateSlashes(param.getFilePath() + SharingConstant.PATH_SEPARATOR + pathAndName));
        if (!file.exists()) {
